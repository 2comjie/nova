...
go -C "$script_dir" build -o "$tool_dir/protoc-gen-go-rpc" ./cmd/protoc-gen-go-rpc

PATH="$tool_dir:$PATH" protoc \
  --proto_path="$script_dir/proto" \
  --go_out="$script_dir" \
  --go_opt=module=github.com/2comjie/nova \
  --go-rpc_out="$script_dir" \
  --go-rpc_opt=module=github.com/2comjie/nova \
  "$script_dir/proto/network/control.proto" \
